...
// one level above lib/. `new URL('../skills', import.meta.url)` already ends in a
// directory name, so do NOT wrap in dirname() — that would wrongly resolve to the
// package root (which also contains node_modules and would double-count SKILL.md).
// This holds for both local dev (dsh-wuyun-liuqi/lib) and the published package
// (node_modules/@wuyun-liuqi/dsh-wuyun-liuqi/lib).
...
function parseFrontmatter(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!line.trim() || /^\s/.test(line))
            continue; // blank or continuation line (consumed by block scalars)
        ...
        if (bm) {
            // Block scalar: gather following indented / blank lines as the value.
            const folded = bm[1][0] === '>';
            const buf = [];
            i++;
...
async function buildCandidates() {
    ...
    const cands = [...byName.values()].map(({ path, fm, rel }) => {
        const candidate = {
            name: fm['name'],
